...
(function(scope){
  ...
  gulp.task('p-scripts:package:now', function(){
    var jsBundles = [distDir + '/scripts/x-names.min.js'];
  ...
  var scriptsBrowserify = function(env){
    var jsBundles = ['app/scripts/x-names.js'];
...
(function(scope){
  ...
  gulp.task('vulcanize:inline:now',function(){
    return gulp.src(distDir + '/x-names.html', {base: distDir + '/'})
      .pipe($.vulcanize({
  ...
  var vulcanizeWithExternalScriptsAndStyles = function(vulcanizeOptions){
    return gulp.src(tmpDir + '/x-names.html')
